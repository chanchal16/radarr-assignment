        ...
        <div className='flex flex-row flex-wrap gap-4'>
        {
            characterList.map((character,index)=>(
                <div key={index} className='w-40 border border-solid border-zinc-700 p-4'>
                    <img src={character.person.imgurl} alt={character.name} className='w-20' /> 
                    <h3 className='text-xl'> {character.name}</h3>
                    <div className='flex'>
                        <button onClick={()=>getInfo(character.person.house,character.name)}>info</button>
                        <button>play</button>
                    </div>
                </div>
            ))
